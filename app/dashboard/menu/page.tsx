      ...
      <div className="max-w-3xl mx-auto p-6">
        ...
        <div className="bg-white rounded-2xl border p-4 mb-6 shadow-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {categories.length === 0 && <span className="text-sm text-gray-400">No categories yet. Add categories to organize your menu items.</span>}
            {categories.map(cat => (
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border p-6 mb-6 shadow-sm">
            <h3 className="font-semibold text-gray-800 mb-4">{editingId ? 'Edit Item' : 'New Item'}</h3>
              ...
              {catItems.map(item => (
                <div key={item.id} className={`bg-white rounded-xl border p-4 flex items-center gap-4 ${!item.available ? 'opacity-50' : ''}`}>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <button onClick={() => deleteItem(item.id)} className="text-red-400 hover:text-red-600 text-sm font-medium">Delete</button>
                  </div>
                </div>
              ))}
            </div>
