...
export default function CheckoutModal({ orders, restaurant, onClose, onPaid }: Props) {
  ...
  const calc = useMemo(() => {
    ...
    if (phActive) {
      ...
      reasons.push(`Public Holiday PH +${phPct}%`)
    }
    if (method === 'card' && cardSurcharge > 0) {
      ...
      reasons.push(`Card +${cardSurcharge}%`)
    }
  ...
  async function confirmPayment() {
    if (!calc.enoughCash) { toast.error('Insufficient cash received'); return }
    setSaving(true)

    // Distribute surcharge proportionally across orders
    ...
    toast.success(`Payment complete $${calc.grandTotal.toFixed(2)} ✓`)
      ...
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl p-6 max-h-[92vh] overflow-y-auto">
        ...
        <div className="bg-orange-50 rounded-xl p-3 mb-4 text-sm">
          <div className="font-semibold">
            {tableNumber.startsWith('T') ? `Takeaway ${tableNumber}` : `Table ${tableNumber}`} · {orders.length} order(s)
          </div>
          <div className="text-gray-500 text-xs mt-0.5">{allItems.length} item(s) · Subtotal ${subtotal.toFixed(2)}</div>
        </div>
        ...
        {method === 'card' && (
          <div className="mb-4 bg-blue-50 rounded-xl p-3">
            ...
            <div className="text-xs text-blue-600 mt-1">Default {restaurant.card_surcharge_pct ?? 1.5}% (adjustable on Orders page)</div>
          </div>
        )}

        <label className="flex items-center justify-between mb-4 bg-yellow-50 rounded-xl p-3 cursor-pointer">
          <div>
            <div className="text-sm font-medium text-yellow-800">Public Holiday +{phPct}%</div>
            <div className="text-xs text-yellow-600">Override for this order</div>
          </div>
        ...
        <div className="border rounded-xl p-3 mb-4 space-y-1 text-sm">
          ...
          {calc.surcharge > 0 && (
            <div className="flex justify-between text-gray-600">
              <span>Surcharge ({calc.surchargeReason})</span><span>+${calc.surcharge.toFixed(2)}</span>
            </div>
          )}
          <div className="border-t pt-2 mt-2 flex justify-between font-bold text-lg text-orange-600">
            <span>Total</span><span>${calc.grandTotal.toFixed(2)}</span>
          </div>
        </div>
        ...
        {method === 'cash' && (
          <div className="mb-4">
            ...
            <input type="number" step="0.01" inputMode="decimal" value={cashReceived}
              ...
              placeholder={`e.g. ${Math.ceil(calc.grandTotal / 10) * 10}`}
            ...
            {cashReceived && !calc.enoughCash && (
              <div className="mt-2 text-sm text-red-600">⚠️ Cash received is insufficient</div>
            )}
          </div>
        )}
        ...
        <button onClick={confirmPayment} disabled={saving || !calc.enoughCash}
          ...
          {saving ? 'Processing...' : `✓ Confirm Payment $${calc.grandTotal.toFixed(2)}`}
        </button>
        <p className="text-center text-xs text-gray-400 mt-2">Closing this window will not cancel the order</p>
      </div>
    </div>
  )
}
